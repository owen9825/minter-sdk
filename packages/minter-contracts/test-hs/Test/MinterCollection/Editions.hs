      ...
      withSender admin $ do
        ...
        forM_ (editionRuns `zip` receivers) \(editionRun, rcvs) ->
          distributeEditions editions [editionRun] [rcvs]
            & expectTransferFailure [failedWith $ constant [mt|NO_EDITIONS_TO_DISTRIBUTE|]]
...
originateEditionsContract storage =
  originateSimple "editions" storage
    =<< importContract (inBinFolder "fa2_multi_nft_token_editions.tz")
